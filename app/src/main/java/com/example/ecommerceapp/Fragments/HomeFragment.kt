...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        //Function call to fetch all shoes from api
        fetchShoes()
    ...
    private fun fetchShoes() {
        ...
        api.getShoes().enqueue(
            object : Callback<ShoesResponse> {
                override fun onResponse(
                    call: Call<ShoesResponse>,
                    response: Response<ShoesResponse>
                ) {
                    //Checking if response is successful
                    if (response.isSuccessful) {
                        val shoesResponse = response.body()
                        if (shoesResponse != null && shoesResponse.result.isNotEmpty()) {
                            allShoes = shoesResponse.result.flatMap { it.products }
                        ...
                        Toast.makeText(context, "API response not successful", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
                ...
                override fun onFailure(call: Call<ShoesResponse>, t: Throwable) {
                    Toast.makeText(
                        context,
                        "Failed to fetch Shoes Data: ${t.message}",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            })
